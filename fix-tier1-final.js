// FINALE Tier 1 Zuordnung - NUR Wikipedia-Hauptbegriffe
...
const tier1Keys = [
  ...
  'variants_family_coiffeur_schieber',   // Coiffeur (RICHTIG)
  // Molotow existiert nicht in DB
  ...
  'trumpf_bauer',               // Puur (RICHTIG - Key ist trumpf_bauer)
];
...
const wikidataIds = {
  ...
  'variants_family_coiffeur_schieber': 'Q137731700',
};

console.log('🔍 FINALE Tier 1 Zuordnung (nur Wikipedia)...\n');
...
for (const [key, entry] of Object.entries(content)) {
  ...
  if (tier1Keys.includes(key)) {
    ...
    tier1Count++;
    console.log(`   ✓ Tier 1: ${entry.metadata?.category?.topic}`);
  } else {
    // Tier 2: Varianten und wichtige Begriffe
    const keyLower = key.toLowerCase();
    if (keyLower.includes('variants_') || 
        keyLower.includes('schieber_') ||
